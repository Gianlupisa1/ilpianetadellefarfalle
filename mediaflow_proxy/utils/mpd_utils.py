...
def parse_mpd_dict(
    ...
    # --- PATCH: forza qualità max ---
    video_profiles = [p for p in profiles if "video" in p["mimeType"]]
    audio_profiles = [p for p in profiles if "audio" in p["mimeType"]]

    if video_profiles:
        best_video = max(video_profiles, key=lambda p: p.get("bandwidth", 0))
        profiles = audio_profiles + [best_video]
    # --------------------------------
